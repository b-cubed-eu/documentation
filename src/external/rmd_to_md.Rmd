...
```{r}
yaml <- yaml::read_yaml(here::here("src", "external", "vignettes.yml"))
```
...
# Function to create absolute path for local files and keep URLs unchanged
resolve_source <- function(source) {
  if (grepl("^https?://", source)) {
    source
  } else {
    file.path(here::here(), source)
  }
}

# Convert (R)md to md for each source of the package
purrr::walk(
  yaml_package,
  ~ b3doc::rmd_to_md(
    resolve_source(.x$source),
    md_dir,
    fig_dir,
    fig_url_dir,
